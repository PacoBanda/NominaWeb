...
document.addEventListener("DOMContentLoaded", async () => {
  usuarioId = await obtenerUsuarioActual();
  if (!usuarioId) return;
  ...
  document.addEventListener("change", async (e) => {
    ...
    if (target.classList.contains("input-fecha-columna")) {
      ...
      if (docOld) {
        await setDoc(doc(db, "usuarios", usuarioId, "PreciosFijos", newId), {
          ...docOld.data(),
          fechaVigor: newId
        });
  ...
  document.getElementById("btnAñadirFijo")?.addEventListener("click", () => {
    ...
    setDoc(doc(db, "usuarios", usuarioId, "PreciosFijos", fechaDefault), {
      fechaVigor: fechaDefault,
      valores: {}
    });
  });
  ...
  document.addEventListener("click", async (e) => {
    if (e.target.classList.contains("btn-eliminar-columna")) {
      if (confirm("¿Eliminar esta columna de precios fijos?")) {
        await deleteDoc(
          doc(db, "usuarios", usuarioId, "PreciosFijos", e.target.dataset.fecha)
        );
      }
    }
  });
...
function renderPreciosFijos() {
  ...
  if (!tbody || !thead || !snapPreciosFijos) return;
...
function renderPreciosVariables() {
  ...
  if (!tbody || !snapPreciosVariables) return;
